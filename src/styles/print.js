...
function specks(id, seed, { color, density, frequency = 0.9, threshold = 0.62 }) {
...
function screenprint(p, id, seed) {
  const defs =
    ...
    `</filter>` +
    specks(`${id}-grain-dark`, seed, { color: p.ink, density: 9, threshold: 0.66 }) +
    specks(`${id}-grain-light`, seed + 7, { color: p.paper, density: 9, threshold: 0.68, frequency: 0.7 });
  const overlay =
...
function aged(p, id, seed) {
  const defs =
    ...
    `</radialGradient>` +
    specks(`${id}-grain`, seed + 3, { color: '#5a3e24', density: 2.6 });
...
export function printStyle(style, p, { id, seed = 1 }) {
  const s = Math.abs(Math.round(seed)) % 1000;
  if (style === 'screenprint') return screenprint(p, id, s);
  if (style === 'aged') return aged(p, id, s);
  return { defs: '', overlay: '' };
}
